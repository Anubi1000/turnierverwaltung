...
{
    /// <summary>
    ///     Adds a validation rule to ensure that a property's value has not changed compared to a previous value stored in the
    ///     validation context.
    /// </summary>
    ...
    /// <param name="message">
    ///     The custom error message to use if the property value has changed.
    ///     Defaults to "'{PropertyName}' is not allowed to change."
    /// </param>
    /// <returns>An <see cref="IRuleBuilderOptions{T, TProperty}" /> for chaining further validation rules.</returns>
    /// <remarks>
    ///     This method checks if the property's current value matches its previous value stored under the given
    ///     <paramref name="contextKey" />

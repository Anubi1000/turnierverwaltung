...
const { data, isPending, isError } = useGetClubs(tournamentId);
...
const mappedItems = computed<ItemListItem[] | undefined>(() => {
  ...
  return rawItems.map<ItemListItem>((item) => ({
    ...
    link: {
      ...
      params: { tournamentId: tournamentId, clubId: item.id },
    },
  }));
});

const selectedItemId = computed(
  () => route.params.clubId as string | undefined,
);
...
<template>
  <MainContentContainer class="flex-col">
    <HeaderBar>
      <h2 class="px-2 text-2xl font-medium">{{ strings.club.items }}</h2>
    </HeaderBar>
